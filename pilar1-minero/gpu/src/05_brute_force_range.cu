...
__device__ int ull_to_str(unsigned long long n, unsigned char* out) {
    ...
    unsigned char temp[20]; int len = 0;
...
__device__ bool check_prefix(const unsigned char* hash, const char* prefix, int hex_len) {
    for (int i = 0; i < hex_len; i++) {
        int hn = i % 2 ? (hash[i/2] & 0xf) : ((hash[i/2] >> 4) & 0xf);
        if (hn != hex_digit(prefix[i])) return false;
    }
    return true;
}

__global__ void brute_force_range_kernel(
    const unsigned char* base, size_t base_len,
    const char* prefix, int hex_len,
    unsigned long long range_min, unsigned long long range_max,
...
int main(int argc, char* argv[]) {
    if (argc < 4) {
        printf("Uso: %s <cadena> <prefijo_hex> <min> [max]\n", argv[0]);
        printf("Ej: %s \"hola\" \"0\" 0 1000000\n", argv[0]);
        return 1;
    }

    int hex_len = strlen(argv[2]);
    size_t base_len = strlen(argv[1]);
    unsigned long long range_min = atoll(argv[3]);
    unsigned long long range_max = argc > 4 ? atoll(argv[4]) : ~0ULL;
    ...
    printf("Buscando MD5(\"%s\"+nonce) empiece con \"%s\"\n", argv[1], argv[2]);
    printf("Rango: [%llu, %llu)\n", range_min, range_max);
    printf("Espacio total: %llu nonces\n", range_max - range_min);

    brute_force_range_kernel<<<blocks, threads>>>(
        d_base, base_len, d_prefix, hex_len,
        range_min, range_max, d_found, d_hash
    );
    ...
    if (h_found != ~0ULL) {
        ...
        printf("No se encontró nonce en el rango especificado\n");
    }
